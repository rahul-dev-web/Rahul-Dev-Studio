...
import { ArrowRight, ArrowUpRight, Code2, ExternalLink, Github, Layers3, Menu, MessageCircle, Smartphone, Sparkles, X } from "lucide-react";
    ...
    <main className="rds-page">
      <header className="nav">
        <div className="container nav-inner">
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <button className="mobile-menu" onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle navigation" aria-expanded={menuOpen}>{menuOpen ? <X size={18}/> : <Menu size={18}/>}</button>
          </div>
        </div>
        {menuOpen && <div className="mobile-nav"><a href="#services" onClick={close}>Services</a><a href="#work" onClick={close}>Work</a><a href="#process" onClick={close}>Process</a><a href="#about" onClick={close}>About</a><a href={DOCS_URL} target="_blank" rel="noreferrer" onClick={close}>Client Guide</a></div>}
      </header>
      ...
      <section id="top" className="hero">
        ...
        <div className="container hero-inner">
          ...
          <div className="hero-visual" aria-label="RDS brand visual">
            <div className="orbit"/><div className="orbit-glow"/>
            <div className="visual-label top">CODE / CREATE / DELIVER</div>
            <div className="logo-frame"><img src="/brand/rds-logo.jpg" alt="Rahul Development Studio logo"/></div>
      ...
      <section className="section" id="work">
        <div className="container">
          ...
          <article className="work-card">
            <div className="work-copy"><span className="work-tag">Local Business Demo Studio</span><h3>From discovery to a website customers can actually use.</h3><p>Explore website concepts for restaurants, salons, gyms, clinics, coaching, hospitality, retail, automobile and real-estate businesses — then bring the right direction into a real project.</p><div className="work-points"><span>Industry-led concepts</span><span>Mobile-first</span><span>Direct contact flows</span><span>Business focused</span></div><a className="button button-primary" style={{width:"fit-content",marginTop:28}} href={DEMO_URL} target="_blank" rel="noreferrer">Explore Demo Studio <ExternalLink size={14}/></a></div>
            <div className="work-visual"><img src="/brand/rds-logo.jpg" alt="RDS visual identity"/><div className="work-overlay"><span>RDS / Visual Direction</span><strong>Web • App • Solutions</strong></div></div>
          </article>
        </div>
      </section>
      ...
      <section className="cta" id="contact">
        <div className="container cta-inner"><div><h2>Have an idea worth building?</h2><p>Tell us what you are trying to create. We can start with the requirement, define the right scope and decide the next step.</p></div><div style={{display:"flex",flexWrap:"wrap",gap:10}}><a className="button button-primary" href={WHATSAPP_URL} target="_blank" rel="noreferrer">Start a Project <MessageCircle size={15}/></a><a className="button button-secondary" href={GITHUB_URL} target="_blank" rel="noreferrer"><Github size={14}/> GitHub</a></div></div>
      </section>

      <footer className="footer"><div className="container footer-inner"><div><span className="footer-brand">RAHUL DEVELOPMENT STUDIO</span><span style={{marginLeft:10}}>Web • App • Solutions</span></div><div className="footer-links"><a href="#services">Services</a><a href="#work">Work</a><a href={DOCS_URL} target="_blank" rel="noreferrer">Client Guide</a><a href={GITHUB_URL} target="_blank" rel="noreferrer">GitHub</a><a href={PORTFOLIO_URL} target="_blank" rel="noreferrer">Portfolio</a></div><span>© 2026 RDS</span></div></footer>
    </main>
  );
}

function Capability({ icon, no, title, text }: { icon: React.ReactNode; no: string; title: string; text: string }) {
  return <article className="capability"><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",color:"#657083"}}><span className="index">{no}</span><span>{icon}</span></div><h3>{title}</h3><p>{text}</p></article>;
}
